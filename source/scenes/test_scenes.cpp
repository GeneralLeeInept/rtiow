...
Scene triangleTest(std::string_view skyhdri)
{
    Scene scene{};
    scene.sky = makeStandardSky(skyhdri);
    scene.cameraCreateInfo.position = Vec3(0, 0.01, -2);
    scene.cameraCreateInfo.target = Vec3(0, 0.4, 0);
    scene.cameraCreateInfo.vup = Vec3(0, 1, 0);
    scene.cameraCreateInfo.fovy = degToRad(30.0);
    scene.cameraCreateInfo.focalDistance = length(scene.cameraCreateInfo.position - scene.cameraCreateInfo.target);
    scene.cameraCreateInfo.aperature = 0.1;

    auto teapotMaterial = std::make_shared<Lambertian>(CornellBoxData::white);
    auto teapot = std::make_shared<TriangleMesh>(teapotMaterial);

    if (!teapot->load(R"(R:\assets\models\teapot.obj)"))
    {
        std::cerr << "Teapot where?\n";
        exit(EXIT_FAILURE);
    }

    Mat4 xform = glm::rotate(degToRad(255), Vec3(0, 1, 0));
    scene.add(std::make_shared<Transform>(xform, teapot));

    auto groundMaterial = std::make_shared<Lambertian>(Vec3(0.48, 0.83, 0.53));
    auto ground = std::make_shared<RectangleXZ>(-2.5, 2.5, -2.5, 2.5, 0, groundMaterial);
    scene.add(ground);
...
Scene teapot(std::string_view skyhdri)
{
    ...
    // Glass tapot
    auto teapotMaterial = std::make_shared<Dielectric>(1.5);
    auto teapot = std::make_shared<TriangleMesh>(teapotMaterial);

    if (!teapot->load(R"(R:\assets\models\teapot_large.obj)"))
    {
        std::cerr << "Teapot where?\n";
        exit(EXIT_FAILURE);
    }

    xform = glm::translate(Vec3(200, 165, 280)) * glm::rotate(degToRad(45), Vec3(0, 1, 0));
    scene.add(std::make_shared<Transform>(xform, teapot));
    ...
    auto material1 = std::make_shared<Dielectric>(1.5);
    auto teapot1 = std::make_shared<TriangleMesh>(material1);

    if (!teapot1->load(R"(R:\assets\models\teapot_medium.obj)"))
    {
        std::cerr << "Teapot where?\n";
        exit(EXIT_FAILURE);
    }

    Mat4 xform = glm::rotate(degToRad(180), Vec3(0, 1, 0));
    scene.add(std::make_shared<Transform>(xform, teapot1));

    auto material2 = std::make_shared<Lambertian>(Vec3(0.4, 0.2, 0.1));
    auto teapot2 = std::make_shared<TriangleMesh>(material2);

    if (!teapot2->load(R"(R:\assets\models\teapot_medium.obj)"))
    {
        std::cerr << "Teapot where?\n";
        exit(EXIT_FAILURE);
    }

    xform = glm::translate(Vec3(-4, 0, 0)) * glm::rotate(degToRad(180), Vec3(0, 1, 0));
    scene.add(std::make_shared<Transform>(xform, teapot2));

    auto material3 = std::make_shared<Metal>(Vec3(0.7, 0.6, 0.5), 0.0);
    auto teapot3 = std::make_shared<TriangleMesh>(material3);

    if (!teapot3->load(R"(R:\assets\models\teapot_medium.obj)"))
    {
        std::cerr << "Teapot where?\n";
        exit(EXIT_FAILURE);
    }
    ...
    xform = glm::translate(Vec3(4, 0, 0)) * glm::rotate(degToRad(180), Vec3(0, 1, 0));
    scene.add(std::make_shared<Transform>(xform, teapot3));
